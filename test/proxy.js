...
describe('HermesProxy', () => {
    ...
    before(async () => {
        ...
        const fundIsPrivate = false;
        await communityStore.connect(wizard).setManager(
            crpPoolAddr,
            wizard.address,
            feesManager,
            feesReferral,
    ...
    it('Can join with unwrapped tokens', async () => {
        ...
        await proxy.joinswapExternAmountInWithSwap(
            crpPoolAddr,
            wAVAXtoken,
            balanceWA.div(100),
            wAVAXtoken,
    ...
    it('Not should be able to join if pool is private and investor not allowed', async () => {
        await communityStore.connect(wizard).setPrivatePool(crpPoolAddr, true);
        await expect(proxy.joinswapPoolAmountOut(
            crpPoolAddr,
            yyAVAXonAAVE,
            hre.ethers.utils.parseEther('1.0'),
            '1000000000000000000',
    ...
    it('Can join private pool with underlying tokens and investor is allowed', async () => {
        await communityStore.connect(wizard).setInvestor(crpPoolAddr, multisig, true);
        const Token = await hre.ethers.getContractFactory('TToken', signer);
        const yyAVAX = await Token.attach(yyAVAXonAAVE);
        const yyPNG = await Token.attach(yyPNGonPangolin);
        const yyUSDCe = await Token.attach(yyUSDCEonPlatypus);
    ...
    it("should be able update wrapper token", async () => {
        const ANY_ADDRESS = '0x8729438EB15e2C8B576fCc6AeCdA6A148776C0F5';
        const ANY_WRAP_ADDRESS = '0xbF5bFFbf7D94D3B29aBE6eb20089b8a9E3D229f7';
        ...
        const wrappers = await proxy.wrappers(crpPoolAddr, ANY_ADDRESS);

        assert.isTrue(wrappers.deposit == ethers.utils.keccak256(ethers.utils.toUtf8Bytes("deposit(uint256)")).slice(0, 10));
        assert.isTrue(wrappers.withdraw == ethers.utils.keccak256(ethers.utils.toUtf8Bytes("withdraw(uint256)")).slice(0, 10));
        assert.isTrue(wrappers.exchange == ethers.utils.keccak256(ethers.utils.toUtf8Bytes("getSharesForDepositTokens(uint256)")).slice(0, 10));
    })
});
